...
test.describe("Learning Path - Micro-Lesson Shell Tests", () => {
  ...
  test("1. Clicking a card opens the MicroLessonShell with complete card context", async ({ page }) => {
    ...
    await expect(shell.locator("[data-testid='lesson-card-title']")).toContainText("Self-Introduction Practice");
    await expect(shell.locator("[data-testid='lesson-learner-instruction']")).toContainText("Give a very brief self-introduction aloud.");
    await expect(shell).toContainText("Ucapkan perkenalan singkat");
    await expect(shell).toContainText("Hello, my name is Alex. I am from Jakarta.");
    await expect(shell).toContainText("Hello, my name is [Name]. I am from [City].");
    await expect(shell).toContainText("Est: 3 mins");
  });
  ...
  test("3. Interactivity - Mark as viewed updates progress safely", async ({ page }) => {
    ...
    await shell.locator("[data-testid='close-lesson-btn']").click();
    await expect(day5Card.locator("span", { hasText: "current" })).toBeVisible();
  });
  ...
  test("8. Storage: Progress values are saved strictly under the correct key", async ({ page }) => {
    ...
    expect(parsed.cards["card-d5-c1"].status).toBe("viewed");
    expect(parsed.cards["card-d5-c1"].attemptCount).toBeDefined();
