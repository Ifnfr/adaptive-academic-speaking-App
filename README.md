...
- **Profile & Settings**: The UI is owner-only and stores profile preferences in Supabase when signed in. `public_profile_enabled` and `leaderboard_opt_in` default to false, and public profile/leaderboard surfaces are still future work. Language preferences are visible as coming-soon placeholders. Learning stats shown in the profile view are local count summaries only, derived from the browser source of truth. Empty display name/bio normalization is a minor polish item for a later pass.
