...
- **Learning Path (Phase 1 & Phase 2)**: A local-first, static-curriculum guided journey.
  - **Phase 1: Confidence Foundation**: A 14-day progressive journey through Unit 1 (Introduce Yourself) and Unit 2 (My Daily Life) with a "Today's Mission" focus. Includes `Guided Word`, `Phrase Pattern`, `Sentence Builder`, and a basic simulated `Micro Speaking` lesson flow.
  - **Phase 2: Everyday Interaction**: Extends the Learning Path sequentially to Days 15–28 with scaffolded everyday interaction practice. Learners practice asking/answering simple questions (Unit 3), expressing preferences (Unit 4), giving one reason, and reviewing safely through fixed-choice and guided speaking activities. Activities include Fluency Sprint, Progressive Sentence Builder, Supported Conversation, Pronunciation Awareness, and Reflection Card.
  - Safe, deterministic local progress and recommendation engine stored in `fonetik:learning-path-progress:v1` localStorage key.
  - Phase 2 remains strictly privacy-safe: no free speaking, open roleplay, generative AI conversation, AI scoring, grammar/pronunciation scoring, or speech-to-text.
  - Privacy-first: Stores only safe progress (card IDs, statuses, attempt counts, timestamps) locally. Never stores transcripts, recordings, audio blob uploads, raw usage, email, or owner IDs.
  - MVP limitations: No adaptive unlocking, no Tutor Memory-driven sequencing, and no Supabase sync yet.
