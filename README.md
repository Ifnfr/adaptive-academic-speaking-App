...
- **Profile & Settings**: Owner-only profile/settings view replacing the old Settings placeholder. Signed-out users see a local-only profile card and safe local stats. Signed-in users see account/profile details, private account email, avatar, display name, bio, privacy toggles, local stat summaries, and language preference controls (App Language and Feedback Language). Profile preferences (including preferredAppLanguage, feedbackLanguage, and targetLanguage) are persisted for signed-in users.
...
- Profile & Settings is owner-only. It is not a public profile page, does not add leaderboard/public-profile surfaces, and never publishes or renders transcripts, retry transcripts, vocabulary sentence history, AI corrections, article URLs, weaknesses, retry tasks, CSV/session raw content, XP event source IDs, or private notes.
